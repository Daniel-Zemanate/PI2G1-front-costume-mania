    ...
    return (
        <>
            <div className="w-64 h-96 bg-white shadow-md rounded-xl duration-500 hover:scale-105 hover:shadow-xl">
                <Image src={costume.url_image} className="h-48 w-80 object-scale-down rounded-t-xl" alt={costume.name} height={320} width={288} />
                <div className="px-4 py-3 w-64">
                    <p className="text-lg font-bold text-black truncate block capitalize">{costume.name}</p>
                    <div className="flex items-center">
                        <p className="text-lg text-black cursor-auto my-3">${costume.price}</p>
                    </div>
                    <div className="flex items-center">
                        {costume.sizes?.map((item) => (
                            <div key={item.id} className="bg-white dark:bg-white-500 text-violet-500 dark:text-violet dark:border-violet-500 dark:border dark:rounded py-1 px-2 mr-2 text-sm">{item.no_size}</div>
                        ))}
                    </div>
